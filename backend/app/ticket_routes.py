...
def confirm_ticket(
    ...
    # 2. Validar propietaris de tots els productes abans de persistir cap
    for idx, prod_item in enumerate(data.productes):
        if prod_item.id_propietaris_privats:
            _, owner_error = _validate_owner_list(
                home.id,
    ...
    for prod_item in data.productes:
        # Categoria → fila a BD
        ...
        catalog_product = (
            ...
            .filter(CatalogProduct.nom == prod_item.nom)
            .first()
        )
        ...
        if prod_item.id_propietaris_privats:
            owner_ids_normalized, _ = _validate_owner_list(
                home.id,
                prod_item.id_propietaris_privats,
                db,
            )
