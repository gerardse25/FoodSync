...
@pytest.mark.parametrize(
    ...
    [
        ("member", "member1_private", -1),  # member consume su privado
        ("member", "member1_private", 1),   # member añade a su privado
        ("member", "public_product", -1),   # member consume público
        ("member", "public_product", 1),    # member añade a público
        ("owner", "owner_private", -1),     # owner consume su privado
        ("owner", "owner_private", 1),      # owner añade a su privado
        ("owner", "public_product", -1),    # owner consume público
        ("owner", "public_product", 1),     # owner añade a público
    ],
)
def test_user_can_modify_allowed_product_quantities(
    ...
    # Evitamos pedir una resta inválida por debajo de 0
...
@pytest.mark.parametrize(
    "who,target_key",
    [
        ("member", "owner_private"),   # member intenta tocar privado del owner
        ("owner", "member1_private"),  # owner intenta tocar privado del member
    ],
)
...
def test_decreasing_more_than_available_stock_returns_error(
    ...
    # En shared_home_with_products el público tiene quantity=3
...
def test_quantity_up_to_max_99_is_allowed(
    ...
    # Creamos un producto controlado para este test
...
def test_other_member_sees_updated_quantity_in_persisted_home_state(
    ...
    """
    Adaptación para la rama 41:
    no hay GET /inv/, así que validamos el estado persistido de la home tras modificar.
    """
