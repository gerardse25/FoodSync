...
def test_inventory_response_includes_expected_basic_product_information(
    ...
    for product in body["productes"]:
        ...
        for owner in product["propietaris"]:
            assert "id_usuari" in owner
            assert isinstance(owner["id_usuari"], str)

            assert "nom" in owner
            assert isinstance(owner["nom"], str)
            assert owner["nom"] not in ("", None)